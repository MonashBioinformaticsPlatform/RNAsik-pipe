...
parser = argparse.ArgumentParser(usage='%(prog)s --dir <path/to/your_data>',
                                 description="scripts for making URL links that you can use to view your data in IGV. Several file types are supported",
                                 add_help=True
                                 )
...
parser.add_argument('--coord',
                    help="this will be your default coordinates when IGV loads for the first time when IGV loads. Pass coordinates as a string as follows chr12:24565477-24624103"
                    )
...
parser.add_argument('--genome_id',
                    help="specify your species genome id. Either IGV inbuilt onces or your own, custome made. Note if custome made genome id is used, you'll need to cache .genome file first, see --add_genome_url options for more."
                    ),
parser.add_argument('--add_genome_url',
                    help="specify url to your .genome file, can do file:///"
                    ),
parser.add_argument('--igv_meta',
                    type=str2bool,
                    nargs='?',
                    default=True,
                    help="Add meta information, describes in brief what IGV is and a coupel of links to IGV resources"
                    )
...
allowed_files = ['bam', 'gtf', 'gff3', 'vcf', 'fa', 'fasta', 'wig', 'bw', 'bedGraph']
...
if igv_meta:
    ...
    igv_intro = '## Intro'
    igv_info = '''It is best to launch local instance of IGV for optimal performance. However it is 
    possible to launch over the web. To launch over the web click on "launch IGV" link below,
    alternatively click on "download/help" link and get your own copy of IGV. 
    Once you have IGV running simply click on IGV link below to load the data into the IGV.
    Please note that you don\'t have to download files for IGV to work. IGV will get the right
    information without you downloading the files, they are there simply for your own reference'''
...
if genome_url is not None:
    ...
    print('<tr><td>Add custome genome to local cache</td><td>Download IGV genome file</td><tr>')
    print('<tr><td><a href="%s">%s</a></td><td><a href="%s">%s</a></td></tr>' % (igv_port+cmd_load+cmd_genome+genome_url, genome_name, genome_url, genome_name))
    print("</table>")
...
for f in sorted(list_of_files):
    ...
    if extn in allowed_files:
        ...
        print('<tr><td><a href="%s">%s</a></td><td><a href="%s">%s</a></td></tr>' % (igv_link, name, my_url, f))
print("</table>")
